...
const parseTime = (time: number): string => (time.toString().length === 1 ? `0${time}` : time.toString());
...
export const Counter = () =>
{
  ...
  return (
    <span>
      {years}y {months}m {days}d, {parseTime(hours)}:{parseTime(minutes)}:<strong>{parseTime(seconds)}</strong>
    </span>
  );
};
